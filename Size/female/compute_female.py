...
from Size.female.size_data_female import handle_split_data
import csv
...
def TrainModel(model, criterion, optimizer, train_loader, test_loader, epochs=1000):
    ...
    for i in range(epochs):
        ...
        if valid_loss <= valid_loss_min:
            ...
            torch.save(model.state_dict(), 'size_female_dutch.pt')
...
def test_model(test_loader,features_column,target_column):
    ...
    test_model.load_state_dict(torch.load("size_female_dutch.pt"))
    test_model.eval()
    test_loss = 0.0
    avg_values = []
    all_pred = []
